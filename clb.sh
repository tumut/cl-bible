#!/bin/sh
# cl-bible: Read the Word of God in any translation from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data "${SOURCE}" | awk -v cmd=list "$(get_data ${AWK_FILE})"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data "${SOURCE}" | awk -v cmd=ref -v ref="$ref" "$(get_data ${AWK_FILE})" | ${PAGER}
	done
	exit 0
fi

get_data "${SOURCE}" 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data ${AWK_FILE})" 2>/dev/null | ${PAGER}
